...
namespace caffe {
	...
	void LBNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (this->phase_ == TRAIN) {
		    ...
		    // save history mean
		    caffe_gpu_axpby(batch_statistic_.count(), Dtype(1) - momentum_, batch_statistic_.gpu_data(), momentum_,
        ...
        // add eps
		caffe_gpu_add_scalar(batch_statistic_.count(), eps_, batch_statistic_.mutable_gpu_data());
		// std
	...
	template <typename Dtype>
	void LBNLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
	...
	INSTANTIATE_LAYER_GPU_FUNCS(LBNLayer);
