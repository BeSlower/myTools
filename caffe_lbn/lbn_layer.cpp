...
#include "caffe/layers/lbn_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void LBNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	// extract param
	eps_ = this->layer_param_.lbn_param().eps();
	momentum_ = this->layer_param_.lbn_param().momentum();
	moving_average_ = this->layer_param_.lbn_param().moving_average();
	...
	if (this->blobs_.size() > 0) {
		...
		shared_ptr<Filler<Dtype> > scale_filler(GetFiller<Dtype>(
			this->layer_param_.lbn_param().scale_filler()));
		...
		shared_ptr<Filler<Dtype> > shift_filler(GetFiller<Dtype>(
			this->layer_param_.lbn_param().shift_filler()));
...
template <typename Dtype>
void LBNLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
	num_ = bottom[0]->num();
	channels_ = bottom[0]->channels();
	height_ = bottom[0]->height();
...
template <typename Dtype>
void LBNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	const vector<Blob<Dtype>*>& top) {
	...
	if (this->phase_ == TRAIN) {
		caffe_cpu_axpby(batch_statistic_.count(), Dtype(1) - momentum_, batch_statistic_.cpu_data(), momentum_,
	...
	// add eps
	caffe_add_scalar(batch_statistic_.count(), eps_, batch_statistic_.mutable_cpu_data());
	// std
...
template <typename Dtype>
void LBNLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->phase_ == TEST && moving_average_) {
	...
	INSTANTIATE_CLASS(LBNLayer);
	REGISTER_LAYER_CLASS(LBN);
